...
custom_log_dir = os.path.join(log_dir, "custom_metrics")
...
num_cpu = 4
custom_writer = SummaryWriter(log_dir=custom_log_dir)
# Define the reward function with writer and epoch_count
def reward_function(obs, act):
    return hover_reward(obs, act, writer=custom_writer, epoch_count=epoch_count, weights={'x': 1, 'v': 0.1, 'w': 0, 'u': 1e-5})
...
def signal_handler(signum, frame):
    print("Interrupt received, closing writer...")
    custom_writer.close()
    sys.exit(0)
