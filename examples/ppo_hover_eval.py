...
def upload_animations_to_tensorboard(folder_path):
    ...
    if not os.path.exists(log_dir + "/videos"):
        os.makedirs(log_dir+"/videos", exist_ok=True)
        print("videos directory has not yet been created and now was created.")
    ...
    frames_np = np.transpose(frames, (0, 3, 1, 2))
    frames_np = np.expand_dims(frames_np, 0)
    log_writer.add_video("my_animation", frames_np, fps=10)
...
def evaluate(auto_mode=False):
    ...
    #plt.show() Commented out during WIP

    # Generating video to upload to tensorboard
    newest_evaluation = get_newest_output_folder_path(output_dir)
    upload_animations_to_tensorboard(newest_evaluation)
